...
public class TransactionService {
    ...
    public void createTransactionFromRequest(TransactionRequest request) {
        Transaction transaction = new Transaction();
        transaction.setDate(request.getDate());
        transaction.setAssetSymbol(request.getAssetSymbol());
        transaction.setQuantity(BigDecimal.valueOf(request.getQuantity()));
        transaction.setTransactionType(TransactionType.valueOf(request.getTransactionType()));
        transaction.setAssetType(AssetType.valueOf(request.getAssetType()));
        transaction.setCurrency(request.getCurrency());
        transaction.setExchange(request.getExchange());
        transaction.setAssetName(request.getAssetName());
        transaction.setPricePerUnit(request.getPricePerUnit() != null ? BigDecimal.valueOf(request.getPricePerUnit()) : null);
        transaction.setTotalAmount(request.getTotalAmount() != null ? BigDecimal.valueOf(request.getTotalAmount()) : null);
        transaction.setCreatedAt(request.getCreatedAt() != null ? request.getCreatedAt() : LocalDate.now());
